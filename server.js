...
app.get('/api/presence', authMiddleware, (req, res) => {
  res.json({ onlineUserIds: [...onlineUserIds] });
});
...
// ---------- Presence (who's online right now) ----------
// A person can have the app open on more than one device/tab, so we count
// sockets per user and only flip them "offline" once the last one disconnects.
const onlineUserIds = new Set();
const socketCountByUser = new Map();

function markOnline(userId) {
  const count = (socketCountByUser.get(userId) || 0) + 1;
  socketCountByUser.set(userId, count);
...
function markOffline(userId) {
  const count = (socketCountByUser.get(userId) || 1) - 1;
  if (count <= 0) {
...
io.on('connection', (socket) => {
  ...
  socket.on('chat:react', ({ messageId, emoji }) => {
    if (typeof messageId !== 'string' || typeof emoji !== 'string' || !emoji) return;
    const message = db.messages.find(m => m.id === messageId);
    if (!message) return;
    if (!message.reactions) message.reactions = {};

    // One reaction per person per message — picking a new emoji replaces
    // their old one; picking the same one again removes it.
    let changed = false;
    const hadThisEmoji = message.reactions[emoji]?.includes(socket.user.id);
    for (const key of Object.keys(message.reactions)) {
      const idx = message.reactions[key].indexOf(socket.user.id);
      if (idx !== -1) {
    ...
    if (!changed) return;
    save();
    io.emit('chat:reaction-update', { messageId, reactions: message.reactions });
  });
